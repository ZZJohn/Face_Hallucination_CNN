...
namespace caffe {
	...
	double tanh_naive(double x) {
		if (x < -40) {
			...
			return ((exp2x - 1.0) / (exp2x + 1.0))*0.5+0.5;
		}
	}

	template <typename TypeParam>
	class My1LayerTest : public MultiDeviceTest<TypeParam> {
	...
	protected:
		...
		virtual ~My1LayerTest() { delete blob_bottom_; delete blob_top_; }
		...
		void TestForward(Dtype filler_std) {
			...
			My1Layer<Dtype> layer(layer_param);
	...
	TYPED_TEST_CASE(My1LayerTest, TestDtypesAndDevices);

	TYPED_TEST(My1LayerTest, TestTanH) {
		this->TestForward(1.0);
	}

	TYPED_TEST(My1LayerTest, TestTanHOverflow) {
		// this will fail if tanh overflow is not properly handled
		this->TestForward(10000.0);
	}

	TYPED_TEST(My1LayerTest, TestTanHGradient) {
