...
#label = label['label']
weights_file = open("weights_bin.pkl", 'rb')
weights = pickle.load(weights_file)
#weights = pickle.load(weights_file, encoding='latin1')
#weights = weights.encode('latin1')
...
all = 10
right = 0
imgmean_file = open('imgavg.pkl', 'rb')
imgmean = pickle.load(imgmean_file)
H, W, C = imgmean.shape
avg = np.zeros((C, H, W))
avg[0, :, :] = imgmean[:, :, 0]
avg[1, :, :] = imgmean[:, :, 1]
